...
public class NotificationQueueWorkerFunction
{
    ...
    {
        var messageId = context.BindingContext.BindingData.TryGetValue("Id", out var id)
            ? id?.ToString() ?? "unknown"
            : "unknown";
        var dequeueCount = context.BindingContext.BindingData.TryGetValue("DequeueCount", out var count)
            ? count?.ToString() ?? "0"
            : "0";

        _logger.LogInformation("Queue trigger function processing message: {MessageId}, DequeueCount: {DequeueCount}",
            messageId, dequeueCount);

        try
        {
            // Deserialize the message
            var notification = JsonSerializer.Deserialize<NotificationMessage>(messageText);

            if (notification == null)
            {
                _logger.LogError(
                    "Failed to deserialize notification message. MessageId: {MessageId}, DequeueCount: {DequeueCount}, MessageText: {MessageText}",
                    messageId, dequeueCount, messageText);
                return;
            }
            ...
            _logger.LogInformation(
                ...
                messageId, notification.Id, dequeueCount, notification.Template, notification.Channel);
            ...
            else
            {
                _logger.LogWarning(
                    ...
                    notification.Id, messageId, dequeueCount, notification.Template, notification.Channel);
                ...
                throw new Exception($"Failed to process notification {notification.Id} (MessageId: {messageId}, DequeueCount: {dequeueCount})");
            }
        }
        ...
        catch (Exception ex)
        {
            _logger.LogError(ex,
                ...
                messageId, dequeueCount, ex.GetType().Name, ex.Message);
